...
struct TheDJson {
	...
	TheDJson(Jsonifier::Document value) {
		Jsonifier::Array valueNew{ value["TEST_VALUE_11"]["d"].valueUnsafe().get_array().valueUnsafe() };
		//std::cout << "ELEMENT COUNT: " << valueNew.countElements().value_unsafe() << std::endl;
		totalTime = 0;
		iterationCount = 0;
		
		for (auto valueIterator = valueNew.begin(); valueIterator != valueNew.end();++valueIterator) {
			...
			strings.emplace_back(valueIterator.operator*().valueUnsafe());
