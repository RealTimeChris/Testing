...
namespace Jsonifier {
	...
	size_t JsonSerializer::size() noexcept {
		switch (this->type) {
			case JsonType::Object: {
				return this->jsonValue.object->size();
			}
	...
	bool JsonSerializer::contains(std::string& key) {
		if (this->type == JsonType::Object) {
			return this->jsonValue.object->contains(key);
		} else {
			return false;
	...
	void JsonSerializer::serializeJsonToEtfString(const JsonSerializer* jsonDataNew) {
	...
	void JsonSerializer::serializeJsonToJsonString(const JsonSerializer* jsonDataNew) {
		switch (jsonDataNew->type) {
			case JsonType::Object: {
	...
	void JsonSerializer::writeString(const char* data, std::size_t length) {
